    ...
    class Shake : ISynapseCommand
    {
        public CommandResult Execute(CommandContext context)
        {
            ...
            if (context.Arguments.Count <= 0)
            {
                ...
                {
                    result.Message = "If you do that you will be stuck with a orange screen until end of the game";
                }
            ...
            else
            {
                ...
                else if (arg.ToLower().Equals("admin"))
                {
                    if (Plugin.Config.ActiveAdminTarget)
                    {
                        ...
                        result.Message = "Admin shaked successfully";
                        result.State = CommandResultState.Ok;
                    }
                    else
                    {
                ...
                else
                {
                    ...
                    if (Count == 1)
                    {
                        result.Message = "Player shaked successfully";
                        result.State = CommandResultState.Ok;
                    }
                    else if (Count > 1)
                    {
                        result.Message = "Players shaked successfully";
                        result.State = CommandResultState.Ok;
                    }
                    else
                    {
